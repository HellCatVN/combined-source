...
try {
  ...
  const untrackedFiles = execSync('git ls-files --others --exclude-standard', { encoding: 'utf8' })
    ...
    .filter(file => !file.startsWith('bm/')); // Exclude files in bm/* directory

  if (untrackedFiles.length === 0) {
    console.log('No untracked files found (excluding bm/* directory)');
  } else {
    console.log('Untracked files (excluding bm/* directory):');
